...
class AnchorBox(object):
  ...
  def __call__(self):
    boxes = []
    for idx, feature_map_size in enumerate(self.feature_maps):
      for i, j in product(range(feature_map_size), repeat=2):
        # the relative coordinate of an anchor box is (i + 0.5) * step_size / img_size
        cx = (j + 0.5) * self.steps[idx] / self.image_size
        cy = (i + 0.5) * self.steps[idx] / self.image_size

        # anchor boxes with size = min_size and aspect_ratio = 1
        s_k = self.min_sizes[idx] / self.image_size
        boxes += [cx, cy, s_k, s_k]

        # anchor boxes with size = sqrt(min_size[k] * min_size[k+1]) and aspect_ratio = 1
        s_k_prime = sqrt(self.min_sizes[idx] * self.max_sizes[idx]) / self.image_size
